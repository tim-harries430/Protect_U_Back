...
class CapabilityPolicy:
    ...
    def resolved_protected_roots(self) -> Sequence[Path]:
        ...
        project_control = tuple(
            Path(root).resolve(strict=False) / name
            for root in self.project_roots
            for name in (".phi", ".claude")
        )
        return explicit + project_control
...
def _is_reversible_delete(proposal: CommandProposal) -> bool:
    ...
    if not any(text.startswith(prefix) for prefix in REVERSIBLE_DELETE_PREFIXES):
        return False
...
def _process_target_complete(proposal: CommandProposal, effects: Set[SideEffect]) -> bool:
    ...
    if effects & {SideEffect.WRITE, SideEffect.DELETE, SideEffect.SECRET_ACCESS, SideEffect.AUDIT_CHANGE}:
        return _contains_any(_normalized_command(proposal.command_text), IMPLICIT_CWD_TARGET_TOKENS)
    if SideEffect.NETWORK in effects:
        return False
    return True
